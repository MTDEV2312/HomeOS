...
export default function HouseholdAdminPage() {
  ...
  const handleDeleteHousehold = async () => {
    if (!activeHousehold) return;
    if (!confirm('¿Estás seguro de que deseas eliminar este hogar? Esta acción es irreversible y eliminará todos los datos asociados (miembros, gastos, tareas, compras, etc.).')) return;
    try {
      setDeleting(true);
      await deleteHousehold(activeHousehold.id);
      await refreshHousehold();
      success('Hogar eliminado', 'El hogar ha sido eliminado correctamente.');
      router.push('/household-setup');
    } catch (err: unknown) {
      showError('Error al eliminar el hogar', getErrorMessage(err));
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-xl w-full min-w-0 max-w-4xl mx-auto">
      ...
      {/* ─── Danger Zone (Leave or Delete Household) ─── */}
      {isOwner ? (
        <div className="bg-error-container/10 rounded-xl border border-error/20 p-lg flex flex-col gap-md">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-md">
            <div className="flex items-center gap-md">
              ...
              <div>
                <p className="font-label-md text-label-md text-on-surface">Eliminar este hogar</p>
                <p className="font-body-sm text-body-sm text-on-surface-variant">
                  Esta acción es permanente y eliminará toda la información, tareas, gastos e inventario de este hogar.
                </p>
              </div>
            </div>
            <button
              onClick={handleDeleteHousehold}
              disabled={deleting || members.length > 1}
              className="px-4 py-2 rounded-lg bg-error text-on-error hover:bg-error/90 transition-colors font-label-md text-label-md disabled:opacity-50 disabled:bg-surface-variant disabled:text-on-surface-variant flex items-center gap-2 shrink-0"
            >
          ...
          {members.length > 1 && (
            <div className="flex items-center gap-2 text-error font-body-sm text-body-sm bg-error-container/20 p-sm rounded-lg border border-error/10">
              <ShieldAlert className="w-4 h-4 shrink-0" />
              <span>
                No podés eliminar el hogar porque aún tiene otros miembros. Debés expulsar a todos los miembros en la sección de arriba antes de poder eliminar la residencia.
              </span>
            </div>
          )}
        </div>
