...
- Protected routes: `src/app/(main)/dashboard/*`
- Auth routes: `login`, `signup`, `verify-email`, `invite/[code]`
...
Uses custom `AuthContext` (`src/lib/auth-context.tsx`) wrapping InsForge SDK:
...
- `signupSchema` — name (2-100 chars), email, password (min 6), confirmPassword match
- `otpSchema` — 6 digits
- `householdSchema` — name (2-100 chars)
- `inviteCodeSchema` — 8 chars, alphanumeric

Use `getFieldErrors(schema, data)` to validate and return field errors.

## Household Context

Multi-tenant households via `HouseholdContext` (`src/lib/household-context.tsx`):
- Household-specific data isolation
- `<HouseholdGuard>` component blocks access without household
